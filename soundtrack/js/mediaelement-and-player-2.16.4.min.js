...
mejs.MediaFeatures = {init: function () {
    var f = this, k = document, j = mejs.PluginDetector.nav, c = mejs.PluginDetector.ua.toLowerCase(), b, a, g = ["source", "track", "audio", "video"];
...
mejs.MediaElementDefaults = {mode: "auto", plugins: ["flash", "silverlight", "youtube", "vimeo"], enablePluginDebug: false, httpsBasicAuthSite: false, type: "", pluginPath: mejs.Utility.getScriptPath(["mediaelement.js", "mediaelement.min.js", "mediaelement-and-player.js", "mediaelement-and-player.min.js"]), flashName: "flashmediaelement.swf", flashStreamer: "", enablePluginSmoothing: false, enablePseudoStreaming: false, pseudoStreamingStartQueryParam: "start", silverlightName: "silverlightmediaelement.xap", defaultVideoWidth: 480, defaultVideoHeight: 270, pluginWidth: -1, pluginHeight: -1, pluginVars: [], timerRate: 250, startVolume: 0.8, success: function () {
}, error: function () {
}};
...
mejs.HtmlMediaElementShim = {create: function (e, d) {
    var n = mejs.MediaElementDefaults, k = (typeof(e) == "string") ? document.getElementById(e) : e, h = k.tagName.toLowerCase(), g = (h === "audio" || h === "video"), b = (g) ? k.getAttribute("src") : k.getAttribute("href"), l = k.getAttribute("poster"), f = k.getAttribute("autoplay"), j = k.getAttribute("preload"), m = k.getAttribute("controls"), a, c;
    for (c in d) {
        n[c] = d[c]
    }
    ...
    var s = n.htmlMediaElement, m = 1, l = 1, u = "me_" + n.method + "_" + (mejs.meIndex++), o = new mejs.PluginMediaElement(u, n.method, n.url), g = document.createElement("div"), b, p, j;
    o.tagName = s.tagName;
        ...
        case"vimeo":
            ...
            if (typeof($f) == "function") {
                ...
                q.addEvent("ready", function () {
                    ...
                    q.setVolume = function (w) {
                        q.api("setVolume", w)
                    };
                    q.setMuted = function (w) {
                        if (w) {
...
(function (b, a, d) {
    var c = {locale: {language: (a.i18n && a.i18n.locale.language) || "", strings: (a.i18n && a.i18n.locale.strings) || {}}, ietf_lang_regex: /^(x\-)?[a-z]{2,}(\-\w{2,})?(\-\w{2,})?$/, methods: {}};
...
(function (a) {
    mejs.MepDefaults = {poster: "", showPosterWhenEnded: false, defaultVideoWidth: 480, defaultVideoHeight: 270, videoWidth: -1, videoHeight: -1, defaultAudioWidth: 400, defaultAudioHeight: 30, defaultSeekBackwardInterval: function (b) {
        return(b.duration * 0.05)
    }, defaultSeekForwardInterval: function (b) {
        return(b.duration * 0.05)
    }, setDimensions: true, audioWidth: -1, audioHeight: -1, startVolume: 0.8, loop: false, autoRewind: true, enableAutosize: true, alwaysShowHours: false, showTimecodeFrameCount: false, framesPerSecond: 25, autosizeProgress: true, alwaysShowControls: false, hideVideoControlsOnLoad: false, clickToPlayPause: true, iPadUseNativeControls: false, iPhoneUseNativeControls: false, AndroidUseNativeControls: false, features: ["playpause", "current", "progress", "duration", "tracks", "volume", "fullscreen"], isVideo: true, enableKeyboard: true, pauseOtherPlayers: true, keyActions: [
        {keys: [32, 179], action: function (b, c) {
            if (c.paused || c.ended) {
                b.play()
            } else {
                b.pause()
    ...
    mejs.MediaElementPlayer.prototype = {hasFocus: false, controlsAreVisible: true, init: function () {
        var e = this, f = mejs.MediaFeatures, j = a.extend(true, {}, e.options, {success: function (l, k) {
            e.meReady(l, k)
        }, error: function (k) {
            e.handleError(k)
        ...
        if ((f.isiPad && e.options.iPadUseNativeControls) || (f.isiPhone && e.options.iPhoneUseNativeControls)) {
            ...
            if (f.isAndroid && e.options.AndroidUseNativeControls) {
            } else {
        ...
        var k = this, j = mejs.MediaFeatures, g = f.getAttribute("autoplay"), d = !(typeof g == "undefined" || g === null || g === "false"), b, l;
        if (k.created) {
            return
        } else {
            k.created = true
        ...
        var l = this, d = 0, k = 0, b = l.controls.find(".mejs-time-rail"), j = l.controls.find(".mejs-time-total"), h = l.controls.find(".mejs-time-current"), f = l.controls.find(".mejs-time-loaded"), c = b.siblings(), g = c.last(), e = null;
        ...
        var j = a('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-loading"><span></span></div></div>').hide().appendTo(h), c = a('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-error"></div></div>').hide().appendTo(h), d = a('<div class="mejs-overlay mejs-layer mejs-overlay-play"><div class="mejs-overlay-button"></div></div>').appendTo(h).bind("click", function () {
        ...
        g.addEventListener("loadeddata", function () {
            ...
            if (mejs.MediaFeatures.isAndroid) {
                g.canplayTimeout = window.setTimeout(function () {
                    if (document.createEvent) {
                        var k = document.createEvent("HTMLEvents");
...
(function (a) {
    a.extend(mejs.MepDefaults, {playText: mejs.i18n.t("Play"), pauseText: mejs.i18n.t("Pause")});
    a.extend(MediaElementPlayer.prototype, {buildplaypause: function (g, h, e, c) {
        var j = this, f = j.options, d = a('<div class="mejs-button mejs-playpause-button mejs-play" ><button type="button" aria-controls="' + j.id + '" title="' + f.playText + '" aria-label="' + f.playText + '"></button></div>').appendTo(h).click(function (l) {
            l.preventDefault();
            if (c.paused) {
                c.play()
            } else {
...
(function (a) {
    a.extend(mejs.MepDefaults, {progessHelpText: mejs.i18n.t("Use Left/Right Arrow keys to advance one second, Up/Down arrows to advance ten seconds.")});
    a.extend(MediaElementPlayer.prototype, {buildprogress: function (q, m, j, g) {
        a('<div class="mejs-time-rail"><span  class="mejs-time-total mejs-time-slider"><span class="mejs-time-buffering"></span><span class="mejs-time-loaded"></span><span class="mejs-time-current"></span><span class="mejs-time-handle"></span><span class="mejs-time-float"><span class="mejs-time-float-current">00:00</span><span class="mejs-time-float-corner"></span></span></div>').appendTo(m);
        m.find(".mejs-time-buffering").hide();
        var l = this, w = m.find(".mejs-time-total"), r = m.find(".mejs-time-loaded"), n = m.find(".mejs-time-current"), u = m.find(".mejs-time-handle"), k = m.find(".mejs-time-float"), p = m.find(".mejs-time-float-current"), o = m.find(".mejs-time-slider"), s = function (B) {
            var C = w.offset(), z = w.outerWidth(true), y = 0, A = 0, D = 0, t;
        ...
        var c = function (y) {
            var A = g.currentTime, z = mejs.i18n.t("Time Slider"), x = mejs.Utility.secondsToTimeCode(A), t = g.duration;
...
(function (a) {
    ...
    a.extend(MediaElementPlayer.prototype, {isFullScreen: false, isNativeFullScreen: false, isInIframe: false, buildfullscreen: function (p, o, g, f) {
        if (!p.isVideo) {
            return
        }
        ...
        if (n.media.pluginType === "native" || (!n.options.usePluginFullScreen && !mejs.MediaFeatures.isFirefox)) {
            ...
            var l = null, h = (function () {
                var w = document.createElement("x"), x = document.documentElement, y = window.getComputedStyle, t;
            ...
            if (h && !mejs.MediaFeatures.isOpera) {
                ...
                }, j = {}, q = ["top", "left", "right", "bottom"], s, u, v = function () {
                    var w = b.offset().left - n.container.offset().left, A = b.offset().top - n.container.offset().top, t = b.outerWidth(true), z = b.outerHeight(true), y = n.container.width(), x = n.container.height();
                    for (s in j) {
                        j[s].css({position: "absolute", top: 0, left: 0})
                    }
...
(function (a) {
    a.extend(mejs.MepDefaults, {speeds: ["2.00", "1.50", "1.25", "1.00", "0.75"], defaultSpeed: "1.00", speedChar: "x"});
    a.extend(MediaElementPlayer.prototype, {buildspeed: function (k, l, g, c) {
        var m = this;
        if (m.media.pluginType == "native") {
            var d = null, e = null, b = null, h = '<div class="mejs-button mejs-speed-button"><button type="button">' + m.options.defaultSpeed + m.options.speedChar + '</button><div class="mejs-speed-selector"><ul>';
            ...
            e.on("click", 'input[type="radio"]', function () {
                ...
                d.find("button").html(n + m.options.speedChar);
...
(function (a) {
    a.extend(mejs.MepDefaults, {startLanguage: "", tracksText: mejs.i18n.t("Captions/Subtitles"), hideCaptionsButtonWhenEmpty: true, toggleCaptionsButtonWhenOnlyOne: false, slidesSelector: ""});
    a.extend(MediaElementPlayer.prototype, {hasChapters: false, cleartracks: function (c, b, e, d) {
        if (c) {
            if (c.captions) {
                c.captions.remove()
            }
            ...
            if (c.captionsText) {
                c.captionsText.remove()
            }
            if (c.captionsButton) {
                c.captionsButton.remove()
            }
        }
    ...
    mejs.TrackFormatParser = {webvtt: {pattern_timecode: /^((?:[0-9]{1,2}:)?[0-9]{2}:[0-9]{2}([,.][0-9]{1,3})?) --\> ((?:[0-9]{1,2}:)?[0-9]{2}:[0-9]{2}([,.][0-9]{3})?)(.*)$/, parse: function (h) {
        var f = 0, d = mejs.TrackFormatParser.split2(h, /\r?\n/), c = {text: [], times: []}, b, g, e;
        for (; f < d.length; f++) {
...
(function (a) {
    ...
    a.extend(MediaElementPlayer.prototype, {buildcontextmenu: function (c, b, e, d) {
        ...
        g.contextMenu.find(".mejs-contextmenu-item").each(function () {
            var k = a(this), l = parseInt(k.data("itemindex"), 10), m = g.options.contextMenuItems[l];
    ...
    a.extend(MediaElementPlayer.prototype, {buildpostroll: function (e, c, g, f) {
        ...
        if (typeof b !== "undefined") {
            ...
            d.media.addEventListener("ended", function (h) {
                a.ajax({dataType: "html", url: b, success: function (j, k) {
